...
if [ "${package_option}" = "--cuda" ]; then
    ...
    PROJECT_NAME="Aeiou-GPU"
elif [ "${package_option}" = "--tflite" ]; then
    PROJECT_NAME="Aeiou-TFLite"
...
if [ "${package_option}" = "--cuda" ]; then
    cp ${DS_ROOT_TASK}/DeepSpeech/ds/tensorflow/bazel-bin/native_client/liblibaeiou.so.ifso ${DS_ROOT_TASK}/DeepSpeech/ds/tensorflow/bazel-bin/native_client/libaeiou.so.if.lib
fi
