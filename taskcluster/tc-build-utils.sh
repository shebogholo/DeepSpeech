...
do_deepspeech_python_build()
{
  ...
  if [ "${package_option}" = "--cuda" ]; then
    SETUP_FLAGS="--project_name aeiou-gpu"
  elif [ "${package_option}" = "--tflite" ]; then
    SETUP_FLAGS="--project_name aeiou-tflite"
  fi
...
do_deepspeech_nodejs_build()
{
  ...
  if [ "${rename_to_gpu}" = "--cuda" ]; then
    make -C native_client/javascript clean npm-pack PROJECT_NAME=aeiou-gpu
  else
    make -C native_client/javascript clean npm-pack PROJECT_NAME=aeiou
  fi
...
do_deepspeech_npm_package()
{
  ...
  if [ "${package_option}" = "--cuda" ]; then
    ...
    make -C native_client/javascript clean npm-pack PROJECT_NAME=aeiou-tflite
  else
...
do_nuget_build()
{
  ...
  cp ${DS_TFDIR}/bazel-bin/native_client/libaeiou.so nupkg/build
...
{
  arch=$1
  cp ${DS_TFDIR}/bazel-bin/native_client/libaeiou.so ${DS_DSDIR}/native_client/swift/libaeiou.so
